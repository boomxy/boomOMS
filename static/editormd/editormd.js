...
;(function (factory) {
    ...
    editormd.prototype = editormd.fn = {
        ...
        createInfoDialog: function () {
            ...
            var infoDialogHTML = [
                ...
                "<p style=\"font-size: 0.85em;\">Copyright &copy; 2015 <a href=\"https://github.com/pandao\" target=\"_blank\" class=\"hover-link\">Pandao</a>, The <a href=\"https://github.com/pandao/editormd/blob/master/LICENSE\" target=\"_blank\" class=\"hover-link\">MIT</a> License.</p>",
                "</div>",
    ...
    editormd.markedRenderer = function (markdownToC, options) {
        ...
        markedRenderer.mark = function (text) {
            if (markReg.test(text)) {
                ...
                text = text.replace(markReg, function (e) {
                    // console.log(2,e)
                    return "<mark>" + e.replace(mark_replace_reg, function ($1, $2) {
                        return $2
                    }) + "</mark>"
                })
            }
        ...
        markedRenderer.code = function (code, lang, escaped) {
            ...
            if (lang === "seq" || lang === "sequence") {
                ...
                // var direction = lang.split(">")[1]
                // console.log(direction)
                var time_line_content = ''
                // console.log(code)
                ...
                timeline_code.forEach(function (item, index) {
                    ...
                    if (item.match(/^# /)) { // 时间标题
                        let time_line_title = item.replace('# ', '')
                        time_line_content += `<div class="timeline-item">
                        ...
                        time_line_content += '</div></div></div>'
                        // console.log(time_line_content)
                    } else if (item.match(/^#> /)) {
                        let time_line_label = item.replace('#> ', '')
                        time_line_content += `<span class="timeline-label">
                        <span class="label label-info">${time_line_label}</span>
                    </span>`
                    }else {
                        time_line_content += marked(item)
                        // console.log(time_line_content)
                    }
                })
                var time_line = `<div class="timeline timeline-single-column">${time_line_content}</div>`
                return time_line;
    ...
    editormd.markdownToHTML = function (id, options) { // Markdown 渲染为 HTML
        var defaults = { // 默认属性
            ...
            previewCodeHighlight: true,
            plugin_path: '/static/editormd/lib/'
        };
    ...
    editormd.katexURL = {
        css: "/static/editormd/lib/katex/katex.min",
        js: "/static/editormd/lib/katex/katex.min",
    };
